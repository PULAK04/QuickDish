  ...
  async (
    ...
    if (user.role !== "rider") {
      return res.status(403).json({
        message:
          "Only riders can create rider profile",
      });
    }
...
export const toggleRiderAvailablity =
  TryCatch(
    async (
      req: AuthenticatedRequest,
      res
  ...
  TryCatch(
    async (
      ...
      if (!rider) {
        return res.status(404).json({
          message:
            "Rider not found or unavailable",
        });
      }
      ...
      if (!order) {
        return res.status(409).json({
          message:
            "Order already taken or unavailable",
        });
      }
...
export const broadcastRiderLocation =
  TryCatch(
    async (
      req: AuthenticatedRequest,
      res
      ...
      if (
        ...
        return res.status(400).json({
          message:
            "Valid rider location is required",
        });
      }
      ...
      if (!rider) {
        return res.status(403).json({
          message:
            "Verified rider profile required",
        });
      }
