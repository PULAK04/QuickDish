...
export const getPendingRestaurant = TryCatch(async (_req, res) => {
  const restaurants = await Restaurant.find({ isVerified: false }).sort({ createdAt: -1 });
  res.json({ count: restaurants.length, restaurants });
});
export const getPendingRiders = TryCatch(async (_req, res) => {
  const riders = await Rider.find({ isVerified: false }).sort({ createdAt: -1 });
  res.json({ count: riders.length, riders });
});
export const verifyRestaurant = TryCatch(async (req, res) => {
  const { id } = req.params;
  if (!id || !mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ message: "Invalid restaurant id" });
...
export const verifyRider = TryCatch(async (req, res) => {
  const { id } = req.params;
  if (!id || !mongoose.Types.ObjectId.isValid(id)) return res.status(400).json({ message: "Invalid rider id" });
  const rider = await Rider.findByIdAndUpdate(id, { isVerified: true }, { new: true });
